...
def calculate_similarity(df):
    ...
    distance = pairwise_distances(tfidf_matrix, metric='manhattan')
    return distance


# Obtém uma lista de vídeos recomendados a partir de um título
def get_recommendations(title, distance, df):
    indices = pd.Series(df.index, index=df['Título']).drop_duplicates()

    sim_scores = sorted(list(enumerate(distance[indices[title]])), key=lambda x: x[1], reverse=True)[
...
if __name__ == "__main__":
    ...
    if videos:
        ...
        distance = calculate_similarity(df)
        ...
        recommendations = get_recommendations(title, distance, df)

        ##### MODIFICAR NOME DO ARQUIVO #####
        file_path = './model_cosine_sim_results/3PAIRWISE_MANHATTAN_TODOS.csv'
